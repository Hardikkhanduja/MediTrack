...
function MedicineIconBox({ status }) {
  const bgColor =
    status < 0   ? "#2a1515" :
    status <= 7  ? "#251d08" : "#131320";
  const pillLeft =
    status < 0   ? "#c94444" :
    status <= 7  ? "#b87c10" : "#6060b0";
  const pillRight =
    status < 0   ? "#6a1e1e" :
    status <= 7  ? "#6a4208" : "#303060";

  return (
    <View style={[styles.iconBox, { backgroundColor: bgColor }]}>
      <PillLogo size={14} colorLeft={pillLeft} colorRight={pillRight} rotate="-35deg" />
    </View>
  );
}
...
export default function NotificationScreen({ navigation }) {
  ...
  const thisWeek = sorted.filter((m) => { const d = getStatus(m.expiry); return d >= 0 && d <= 7; });
      ...
      >
        {isEmpty && (
          <View style={styles.emptyState}>
            <Ionicons name="notifications-off-outline" size={48} color="#2a2a38" style={{ marginBottom: 16 }} />
            <Text style={styles.emptyTitle}>No notifications</Text>
            <Text style={styles.emptySubText}>Add medicines to get expiry alerts</Text>
          </View>
        )}

        {pastDue.length > 0 && (
          <View style={styles.section}>
            <SectionHeader dot="#e05555" title="Past Due" count={pastDue.length} />
            <View style={styles.cardGroup}>
              {pastDue.map((item) => <MedicineNotifCard key={item.id} item={item} />)}
            </View>
          </View>
        )}
        ...
        {thisWeek.length > 0 && (
          <View style={styles.section}>
            <SectionHeader dot="#c9940a" title="This Week" count={thisWeek.length} />
            <View style={styles.cardGroup}>
              {thisWeek.map((item) => <MedicineNotifCard key={item.id} item={item} />)}
            </View>
          </View>
        )}
                  ...
                  >
                    <View style={styles.laterCardTop}>
                      <View style={styles.iconBoxLater}>
                        <PillLogo size={14} colorLeft="#6060b0" colorRight="#303060" rotate="-35deg" />
                      </View>
...
const styles = StyleSheet.create({
  ...
  sectionTitle: {
    fontSize: 15,
    fontWeight: "600",   // ← was 700, now lighter
    color: "#ccccdd",    // ← was white, now softer
  },
  ...
  notifCard: {
    flexDirection: "row",
    alignItems: "center",
    padding: 14,         // ← was 16, slightly tighter
    gap: 14,
    borderBottomWidth: 1,
  ...
  cardName: {
    fontSize: 14,        // ← was 15
    fontWeight: "500",   // ← was 700, much lighter now
    color: "#e0e0f0",    // ← slightly off-white, less harsh
    marginBottom: 3,
  },
  cardSub: {
    fontSize: 12,
  ...
  laterTag: {
    ...
    color: "#555568",    // ← was #666680, slightly darker
    letterSpacing: 0.8,
  },
  laterName: {
    fontSize: 14,        // ← was 16
  ...
  emptyTitle: { fontSize: 18, fontWeight: "600", color: "#ccccdd", marginBottom: 8 },
